...
\begin{tabular}{m{3cm} m{11cm} m{3cm}}
    ...
    Fecha de Entrega: 16 y 21 de Abril de 2026 &
    \centering
...
int levantar_servicio(int puerto)
{
    ...
    // si se reinicia el servidor rápido durante pruebas
...
Implementar este motor transaccional cliente-servidor demostró ser un desafío complejo que requirió un diseño relacional riguroso hasta la Cuarta Forma Normal (4NF) para estructurar correctamente las entidades antes de persistirlas en archivos .csv.

El mayor acierto del proyecto fue su diseño modular. Reutilizar el motor de consultas para realizar las verificaciones de integridad (como validar la duplicidad de llaves primarias) optimizó el flujo del código y demostró el valor de una buena arquitectura de software.

Como áreas de mejora futuras, se identifica la necesidad de implementar una sanitización de entradas más estricta (para evitar que caracteres inválidos afecten la ejecución) y desarrollar la capacidad de realizar actualizaciones parciales en la operación UPDATE, evitando la sobreescritura total del registro si hay campos vacíos.

En conclusión, la comunicación bidireccional mediante sockets TCP fue un éxito. Se logró emular el núcleo de un SGBD de forma segura, evidenciando la verdadera complejidad de los sistemas distribuidos y el estricto manejo de memoria a bajo nivel. El siguiente paso para escalar este servidor será la integración de múltiples conexiones simultáneas, lo cual implicará el reto de implementar algoritmos de sincronización de hilos.

\begin{itemize}
    \item \textbf{Mario Mendoza Gonzáles}
\end{itemize}

Desarrollar esta aplicación cliente-servidor fue un reto bastante interesante que nos permitió aterrizar los conceptos teóricos en un entorno real. El proceso de pasar de un simple archivo de Excel a una base de datos estructurada y normalizada hasta la cuarta forma normal (4F)  requirió de un buen análisis inicial, pero fue un paso clave para asegurar que la información fluyera sin redundancias ni errores estructurales.  Implementar la comunicación a nivel de red utilizando sockets TCP  nos ayudó a comprender a fondo cómo viajan y se procesan los datos entre máquinas distintas. Lograr que el cliente y el servidor interactuaran correctamente mediante el uso de máscaras y parámetros para ejecutar todas las operaciones exigidas (consultar, insertar, actualizar y eliminar registros)  demostró la importancia de definir un buen protocolo de comunicación. 
En resumen, el examen se logró con éxito. Logramos conectar la base de datos, que en este caso se trabajó con archivos csv, con la programación en C, entregando un programa capaz de gestionar la base de datos de manera consistente a través de un cable de red.
